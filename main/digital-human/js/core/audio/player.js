...
export class AudioPlayer {
        ...
        try {
            if (typeof window.ModuleInstance === 'undefined') {
                if (typeof Module !== 'undefined') {
                    ...
                    throw new Error('Opus library not loaded, neither ModuleInstance nor Module exists');
                }
            }
            ...
            this.opusDecoder = {
                ...
                decode: function (opusData) {
                    if (!this.decoderPtr) {
                        if (!this.init()) {
                            throw new Error("Decoder is not initialized and cannot be initialized");
                        }
                    }
    ...
    enqueueAudioData(opusData) {
        if (opusData.length > 0) {
            ...
            log('Received an empty audio data frame, possibly an end marker', 'warning');
    ...
    getAudioStats() {
        ...
        return {
            pendingDecode,  // number of packets awaiting decode
            pendingPlay,    // number of packets awaiting playback
            totalPending: pendingDecode + pendingPlay  // total number of pending packets
        };
    }
    ...
    clearAllAudio() {
        ...
        // Clear the receive queue (use the clear method to keep the object reference)
        this.queue.clear();

        // Clear all buffers in the streaming context
        if (this.streamingContext) {
