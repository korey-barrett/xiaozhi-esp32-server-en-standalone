...
export class AudioRecorder {
    ...
    async createAudioProcessor() {
        ...
        try {
            if (this.audioContext.audioWorklet) {
                ...
                log('Processing audio with AudioWorklet', 'success');
                ...
                log('AudioWorklet not available, using ScriptProcessorNode as a fallback', 'warning');
                return this.createScriptProcessor();
            }
        } catch (error) {
            log(`Failed to create audio processor: ${error.message}, trying the fallback`, 'error');
            return this.createScriptProcessor();
        }
    }
    ...
    createScriptProcessor() {
        try {
            ...
            silent.connect(this.audioContext.destination);
            log('Successfully used ScriptProcessorNode as the fallback', 'warning');
    ...
    encodeAndSendOpus(pcmData = null) {
        if (!this.opusEncoder) {
            log('Opus encoder is not initialized', 'error');
            return;
        }
        try {
            if (pcmData) {
    ...
    async start() {
        ...
        try {
            ...
            if (!processorResult) {
                log('Unable to create an audio processor', 'error');
                return false;
            }
...
export async function checkMicrophoneAvailability() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        log('Browser does not support the getUserMedia API', 'warning');
        return false;
    }
    try {
