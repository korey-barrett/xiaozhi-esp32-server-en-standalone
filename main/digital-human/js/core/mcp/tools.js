...
export async function initMcpTools() {
    ...
    if (savedTools) {
        try {
            ...
            // Merge the default tools and the user's saved tools, keeping the user's custom tools
            const defaultToolNames = new Set(defaultMcpTools.map(t => t.name));
            // Add new tools that are not in the default tools
            ...
            log('Failed to load MCP tools, using the default tools', 'warning');
            mcpTools = [...defaultMcpTools];
        }
    } else {
        mcpTools = [...defaultMcpTools];
...
/**
 * Add a parameter - open the parameter edit modal
 */
...
/**
 * Edit a parameter - open the parameter edit modal
 */
...
/**
 * Close the parameter edit modal
 */
...
function setupMcpEventListeners() {
    ...
    // Elements related to the parameter edit modal
...
function openMcpModal(index = null) {
    ...
    if (isConnected) {
        alert('WebSocket is connected, tools cannot be edited');
        return;
    }
...
function handleMcpSubmit(e) {
    ...
    if (isDuplicate) {
        showMcpError('A tool with this name already exists, please use a different name');
        return;
    }
    // Parse the mock response result
...
export async function executeMcpTool(toolName, toolArgs) {
    ...
    // Handle the photo-taking tool
    if (toolName === 'self_camera_take_photo') {
        if (typeof window.takePhoto === 'function') {
            const question = toolArgs && toolArgs.question ? toolArgs.question : 'Describe the object you see';
            ...
            return { success: false, error: 'The camera has not been started or photo capture is not supported' };
        }
    }

    // If there is a mock response, use it
    if (tool.mockResponse) {
        // Replace template variables
        let responseStr = JSON.stringify(tool.mockResponse);
        // Replace variables in the ${paramName} format
        if (toolArgs) {
    ...
    // No mock response, return the default success message
    log(`Tool ${toolName} executed successfully, returned the default result`, 'success');
    return { success: true, message: `Tool ${toolName} executed successfully`, tool: toolName, arguments: toolArgs };
}

// Expose global methods for use by inline HTML event handlers
