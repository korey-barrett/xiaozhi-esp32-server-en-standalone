...
class UIController {
    ...
    initEventListeners() {
        ...
        // Click modal background to close (disabled only for specific modals)
            ...
            modal.addEventListener('click', (e) => {
                if (e.target === modal) {
                    ...
                    if (nonClosableModals.includes(modal.id)) {
                        return; // Prevent closing by clicking the background
                    }
                    this.hideModal(modal.id);
                }
            });
        ...
        // Connect and send buttons are not removed; can be added to the dial button later
    }
    ...
    updateDialButton(isConnected) {
        ...
        if (cameraBtn && !isConnected) {
            ...
            cameraBtn.title = 'Please connect to the server first';
            // Stop camera
            if (typeof window.stopCamera === 'function') {
                window.stopCamera();
            }
        ...
        if (recordBtn) {
            ...
            if (isConnected && microphoneAvailable) {
                ...
                if (!microphoneAvailable) {
                    recordBtn.title = window.isHttpNonLocalhost ? 'Recording is unavailable under HTTP access; you can only interact via text' : 'Microphone unavailable';
                } else {
                    recordBtn.title = 'Please connect to the server first';
                }
                // Restore record button to normal state
                recordBtn.querySelector('.btn-text').textContent = 'Record';
                recordBtn.classList.remove('recording');
    ...
    updateMicrophoneAvailability(isAvailable, isHttpNonLocalhost) {
        ...
        if (!isAvailable) {
            ...
            recordBtn.querySelector('.btn-text').textContent = 'Record';
            recordBtn.title = isHttpNonLocalhost ? 'Recording is unavailable under HTTP access; you can only interact via text' : 'Microphone unavailable';
    ...
    async handleApplyWakeword() {
        ...
        if (payload.enabled && payload.wakeWords.length === 0) {
            this.addChatMessage('When enabling wake words, at least one wake word must be entered.', false);
            return;
        }
        ...
        try {
            ...
            // Compare the new address with the current connection address
            ...
            if (urlChanged) {
                // The address changed, confirm first
                const shouldRestart = window.confirm('The address has changed. Continue? (The old connection will be closed and reconnected)');
                if (!shouldRestart) {
                    // Restore the old address in localStorage
                ...
                // Start the new connection (automatically uses the new address)
                startWakewordBridgeListener();

                // Wait for bridge_connected
                await new Promise((resolve, reject) => {
                    const timeout = setTimeout(() => {
                        reject(new Error('Timed out connecting to the new server'));
                    }, 5000);
                ...
                this.addChatMessage('Wake word config saved, the wake word service is restarting.', false);
            } else {
                // The address is unchanged: operate on the current connection directly
                const response = await requestWakewordBridge('set_wakeword_config', payload);
                this.applyWakewordConfig(response.payload || payload);

                const shouldRestart = window.confirm('Wake words saved. Restart the wake word service now to apply immediately?');
                if (!shouldRestart) {
                    this.addChatMessage('Wake word config saved; it will take effect after manually restarting the service later.', false);
                    return;
                }
    ...
    startAIChatSession() {
        this.addChatMessage('Connection succeeded, let\'s start chatting~😊', false);
        // Check microphone availability and show error messages if needed
        if (!window.microphoneAvailable) {
            if (window.isHttpNonLocalhost) {
                this.addChatMessage('⚠️ Recording is unavailable under HTTP access; you can only interact via text', false);
            } else {
                this.addChatMessage('⚠️ Microphone unavailable, please check permission settings; you can only interact via text', false);
            }
        }
        ...
        if (window.cameraAvailable && typeof window.startCamera === 'function') {
            window.startCamera().then(success => {
                if (success) {
                    ...
                    this.addChatMessage('⚠️ Camera start failed, possibly rejected by the browser', false);
                }
            }).catch(error => {
                log(`Exception starting camera: ${error.message}`, 'error');
            });
    ...
    async handleConnect() {
        ...
        if (this.isConnecting || (wsHandler && wsHandler.isConnected())) {
            log('A connection already exists or is in progress; ignoring this dial request', 'info');
            return;
        }
        ...
        try {
            ...
            if (!otaUrlInput || !otaUrlInput.value) {
                this.addChatMessage('Please enter the OTA server address', false);
                return;
            }
            ...
            const errorMessage = error.message.includes('Cannot set properties of null')
                ? 'Connection failed: please check the device connection'
    ...
    async triggerWakewordDial(wakeWord = 'wake word') {
        ...
        if (this.isConnecting || this.dialBtnDisabled) {
            log('Page is connecting, ignoring duplicate wake', 'info');
            return false;
        }

        if (now - this.lastWakewordDialTime < 3000) {
            log('Wake trigger too frequent, ignoring this auto dial', 'warning');
            return false;
        }
