...
public class AgentSnapshotController {
    ...
    @GetMapping
    @Operation(summary = "Get the agent snapshot list")
    ...
    @Operation(summary = "Get the agent snapshot details")
    @RequiresPermissions("sys:role:normal")
    ...
    @Operation(summary = "Restore the agent snapshot")
    @RequiresPermissions("sys:role:normal")
    ...
    @Operation(summary = "Delete the agent historical snapshot")
    @RequiresPermissions("sys:role:normal")
    ...
    private void checkPermission(String agentId) {
        ...
        if (user == null || !agentService.checkAgentPermission(agentId, user.getId())) {
            throw new RenException("You do not have permission to access this agent's snapshot");
        }
    }
}
