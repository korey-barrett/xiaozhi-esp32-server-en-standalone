...
public class AgentVoicePrintController {
    ...
    @PutMapping
    @Operation(summary = "Update the agent's corresponding voiceprint")
    @RequiresPermissions("sys:role:normal")
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Delete the agent's corresponding voiceprint")
    @RequiresPermissions("sys:role:normal")
    public Result<Void> delete(@PathVariable String id) {
        Long userId = SecurityUser.getUserId();
        // First delete the associated device
    ...
    @GetMapping("/list/{id}")
    @Operation(summary = "Get the voiceprint list of the specified agent for the user")
    @RequiresPermissions("sys:role:normal")
