...
public class AgentSnapshotServiceImpl extends BaseServiceImpl<AgentSnapshotDao, AgentSnapshotEntity>
    ...
    public void restoreSnapshot(String agentId, String snapshotId, String currentStateToken) {
        ...
        if (data == null) {
            throw new RenException("Snapshot data is empty and cannot be restored");
        }
        ...
        if (!Objects.equals(buildCurrentStateToken(currentData), currentStateToken)) {
            throw new RenException("The current configuration has changed; please reopen the restore preview and try again");
        }
        ...
        if (updatedRows < 0 || updatedRows > 1) {
            throw new RenException("Failed to restore the agent snapshot");
        }
    ...
    public void deleteSnapshot(String agentId, String snapshotId) {
        ...
        if (Objects.equals(entity.getVersionNo(), maxVersionNo)) {
            throw new RenException("The latest historical version cannot be deleted");
        }
    ...
    public long redactLegacySnapshots() {
        ...
        while (true) {
            ...
            for (AgentSnapshotEntity snapshot : batch) {
                ...
                if (rawData == null) {
                    throw new RenException("Historical snapshot data cannot be parsed; redaction migration aborted: " + snapshot.getId());
                }
            ...
            if (affected < 0 || affected > batch.size()) {
                throw new RenException("Failed to run the batch redaction migration for historical snapshots");
            }
            migrated += affected;
    ...
    private void insertSnapshot(String agentId, Long userId, String source, AgentSnapshotDataDTO snapshotData,
        ...
        if (inserted != 1) {
            throw new RenException("Failed to generate the snapshot version number");
        }
        if (pruneAfterInsert) {
            pruneSnapshots(agentId);
        }
    ...
    private AgentSnapshotEntity getSnapshotEntity(String agentId, String snapshotId) {
        ...
        if (entity == null || !Objects.equals(agentId, entity.getAgentId())) {
            throw new RenException("Snapshot does not exist");
        }
        return entity;
    }
    ...
    private String restoreTag(AgentSnapshotTagDTO snapshotTag, Date now) {
        ...
        if (tag != null) {
            if (Objects.equals(tag.getDeleted(), 1)) {
                ...
                throw new RenException("The tag referenced by the snapshot has been deleted and cannot be restored; please recreate or select a tag first");
            }
            return tag.getId();
        }
    ...
    private AgentSnapshotDataDTO parseSnapshotData(String value) {
        ...
        try {
            ...
            throw new RenException("Snapshot data cannot be parsed");
        }
    }
    ...
    private void validateSensitiveRestoreIsReversible(AgentSnapshotDataDTO current,
        ...
        try {
            ...
            throw new RenException("The target version removes sensitive configuration that cannot be written to history; please manually handle the related secrets before restoring");
        }
    }
    ...
    private List<Object> preserveCurrentSensitiveList(List<?> target, List<?> current, String parentKey) {
        ...
        for (Object targetItem : target) {
            ...
            if (currentItem == null && containsSensitiveMaterial(targetItem)) {
                throw new RenException("Sensitive list items in the snapshot lack a unique stable identity and cannot be restored safely");
            }
    ...
    private Object preserveSensitiveScalar(Object target, Object current) {
        ...
        throw new RenException("Snapshot sensitive values cannot be reliably matched to the current configuration");
    }
    ...
    private List<ContextProviderDTO> preserveCurrentContextProviders(List<ContextProviderDTO> target,
        ...
        for (ContextProviderDTO targetProvider : target) {
            ...
            if (StringUtils.isNotBlank(identity)
                ...
                throw new RenException("Sensitive context source configuration lacks a unique URL identity and cannot be restored safely");
            }
    ...
    private void validateStructuredSensitiveDiscriminator(Map<?, ?> target, Map<?, ?> current) {
        ...
        if (targetIdentities.size() != 1 || currentIdentities.size() != 1
            ...
            throw new RenException("The type identity of the structured sensitive value in the snapshot cannot be uniquely matched to the current configuration");
        }
    }
    ...
    private String preserveCurrentSensitiveUrl(String target, String current, String semanticKey) {
        ...
        if (copiesSensitiveComponent && !Objects.equals(targetPath.redactedBase(), currentPath.redactedBase())) {
            throw new RenException("Sensitive snapshot URL information cannot be matched to the public address identity of the current configuration");
        }
        ...
        if (result.contains(SECRET_PLACEHOLDER)) {
            throw new RenException("The snapshot URL still contains a redaction placeholder and cannot be restored safely");
        }
        return result;
    }
    ...
    private String preserveUrlComponent(String target, String current) {
        ...
        if (current == null || current.contains(SECRET_PLACEHOLDER)) {
            throw new RenException("Sensitive information in the snapshot URL cannot be reliably matched to the current configuration");
        }
        return current;
    }
    ...
    private String preserveSensitiveUrlParameters(String target, String current) {
        ...
        for (UrlParameter targetParameter : targetParameters) {
            ...
            if (matchingTargets.size() != 1 || matchingCurrent.size() != 1) {
                throw new RenException("Sensitive parameters in the snapshot URL cannot be uniquely matched to the current configuration");
            }
    ...
    private String preserveSensitiveUrlPath(String targetBase, String currentBase, String semanticKey) {
        ...
        if (!Objects.equals(target.redactedBase(), current.redactedBase())
            ...
            throw new RenException("Sensitive path information in the snapshot URL cannot be uniquely matched to the public identity of the current configuration");
        }
        ...
        for (SensitivePathSlot targetSlot : target.slots()) {
            ...
            if (matches.size() != 1 || matches.get(0).secretSegments().isEmpty()
                ...
                throw new RenException("Sensitive path information in the snapshot URL cannot be reliably matched to the current configuration");
            }
            if (replacements.put(targetSlot.identity(), matches.get(0)) != null) {
                throw new RenException("Sensitive path information in the snapshot URL is ambiguous and cannot be restored safely");
            }
        }
    ...
    private String buildSensitiveUrl(String base, String userInfo, String query, String fragment) {
        ...
        if (userInfo != null) {
            ...
            if (authorityStart < 0) {
                throw new RenException("URL format with user credentials is invalid and cannot be restored safely");
            }
