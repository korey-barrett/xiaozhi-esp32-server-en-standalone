...
public class OTAController {
    ...
    public ResponseEntity<String> checkOTAVersion(
        ...
        // The device Id and MAC address should be identical, and the application field is required
        if (!macAddressValid) {
    ...
    public ResponseEntity<String> getOTA() {
        ...
        if (StringUtils.isBlank(mqttUdpConfig)) {
            return ResponseEntity.ok("OTA interface is abnormal: missing mqtt_gateway address. Please log in to the Console, find the [server.mqtt_gateway] configuration under Parameter Management");
        }
        String wsUrl = sysParamsService.getValue(Constant.SERVER_WEBSOCKET, true);
        if (StringUtils.isBlank(wsUrl) || wsUrl.equals("null")) {
            return ResponseEntity.ok("OTA interface is abnormal: missing websocket address. Please log in to the Console, find the [server.websocket] configuration under Parameter Management");
        }
        String otaUrl = sysParamsService.getValue(Constant.SERVER_OTA, true);
        if (StringUtils.isBlank(otaUrl) || otaUrl.equals("null")) {
            return ResponseEntity.ok("OTA interface is abnormal: missing ota address. Please log in to the Console, find the [server.ota] configuration under Parameter Management");
        }
    ...
    /**
     * Simply determine whether a MAC address is valid (non-strict)
     * 
     * @param macAddress
     * @return
     */
    private boolean isMacAddressValid(String macAddress) {
        ...
        // A MAC address is usually 12 hexadecimal digits and may contain colon or hyphen separators
