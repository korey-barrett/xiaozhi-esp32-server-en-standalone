...
public class OtaServiceImpl extends BaseServiceImpl<OtaDao, OtaEntity> implements OtaService {
    ...
    public void update(OtaEntity entity) {
        // Check whether a firmware with the same type and version already exists (excluding the current record)
        QueryWrapper<OtaEntity> queryWrapper = new QueryWrapper<OtaEntity>()
                .eq("type", entity.getType())
                .eq("version", entity.getVersion())
                .ne("id", entity.getId()); // Exclude the current record

        if (baseDao.selectCount(queryWrapper) > 0) {
            throw new RuntimeException("A firmware with the same type and version already exists. Please modify it and try again");
        }
    ...
    public boolean save(OtaEntity entity) {
        QueryWrapper<OtaEntity> queryWrapper = new QueryWrapper<OtaEntity>()
                .eq("type", entity.getType());
        // Only keep the latest record for the same type of firmware
