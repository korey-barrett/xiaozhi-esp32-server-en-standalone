...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    public void configureMessageConverters(List<HttpMessageConverter<?>> converters) {
        // converters for special purposes
    ...
    public MappingJackson2HttpMessageConverter jackson2HttpMessageConverter() {
        ...
        // configure serialization and deserialization for java.util.Date
    ...
    /**
     * Internationalization configuration - sets the locale based on the Accept-Language request header
     */
    @Bean
    public LocaleResolver localeResolver() {
        return new AcceptHeaderLocaleResolver() {
            ...
            public Locale resolveLocale(HttpServletRequest request) {
                ...
                // Parse the preferred language from the Accept-Language request header
                String[] languages = acceptLanguage.split(",");
                if (languages.length > 0) {
                    // Extract the first language code, removing any quality value (q=...)
                    String[] parts = languages[0].split(";" + "\\s*");
                    String primaryLanguage = parts[0].trim();

                    // Create the Locale object directly from the language code sent by the frontend
                    if (primaryLanguage.equals("zh-CN")) {
                        ...
                        // for other German variants, default to German
                        return Locale.GERMANY;
                    } else if (primaryLanguage.startsWith("vi")) {
                        // for other Vietnamese variants, default to Vietnamese
                        return Locale.forLanguageTag("vi-VN");
                    }
                }

                // if no language matches, use the default language
                return Locale.getDefault();
            }
        };
    }
