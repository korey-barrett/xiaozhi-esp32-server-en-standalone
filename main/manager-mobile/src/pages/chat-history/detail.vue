...
// Get the distance from the screen edge to the safe area
...
function getMessageContent(message: ChatMessage): string {
  if (message.chatType === 1) {
    // User message, needs JSON parsing
...
function formatTime(timeStr: string) {
  ...
  // Handle the time string to ensure the format is correct
  ...
  // More than 7 days, show the specific date
...
const playAudio = debounce(async (audioId: string) => {
  ...
  try {
    ...
    // If the current audio ID matches the requested one, pause playback
...
function extractContentFromString(content: string) {
  ...
  // If it is not JSON format or has no content field, return the original content
  return content
}
        ...
        >
          ...
          >
            ...
            >
              ...
              <!-- Content area - use flex layout to align the icon and text -->
...
<style>
/* Custom shadow and animation effects, styles that cannot be represented with UnoCSS */
.shadow-message {
