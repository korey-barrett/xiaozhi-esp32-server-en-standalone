...
// AFSK modulation parameters - reference the HTML file
...
// Convert a string to a byte array - uniapp compatible version
...
// Checksum calculation - reference the HTML file
function checksum(data: number[]): number {
  return data.reduce((sum, b) => (sum + b) & 0xFF, 0)
}

// Convert bytes to bits - reference the HTML file
...
// AFSK modulation - reference the HTML file algorithm
...
// Convert floats to 16-bit PCM - reference the HTML file
...
// Convert an array to base64 encoding - compatible version
function arrayBufferToBase64(buffer: ArrayBuffer): string {
  const bytes = new Uint8Array(buffer)

  // Try native btoa; use the custom implementation if it is unavailable
...
// Build a WAV file - returns an ArrayBuffer instead of a Blob
...
// Generate and play the sound wave - main function
async function generateAndPlay() {
  ...
  try {
    ...
    // Prepare the network config data - reference the HTML file format
    const dataStr = `${props.selectedNetwork.ssid}\n${props.password}`
    const textBytes = stringToBytes(dataStr)
    ...
    // AFSK modulation - reduce the sample rate to lower the file size
    const reducedSampleRate = 22050 // Lower sample rate
    ...
    for (let i = 0; i < bits.length; i++) {
      ...
      for (let j = 0; j < samplesPerBit; j++) {
        ...
        floatBuf[i * samplesPerBit + j] = Math.sin(2 * Math.PI * freq * t) * 0.5 // Lower volume
      }
    }

    const pcmBuf = floatTo16BitPCM(floatBuf)

    // Generate a WAV file - using the reduced sample rate
...
function buildWavOptimized(pcm: Uint8Array, sampleRate: number): ArrayBuffer {
  ...
  write16(20, 1)
  write16(22, 1)
  write32(24, sampleRate) // Use the passed sample rate
...
async function playAudio() {
  ...
  try {
    ...
    // Wait a moment to ensure the cleanup completes
...
<template>
  <view class="ultrasonic-config">
    ...
    <!-- Ultrasonic network config operation -->
