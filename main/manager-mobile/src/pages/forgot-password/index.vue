...
async function handleSendSmsCode() {
  ...
  try {
    ...
    // Call the send SMS verification code API
    await sendSmsCode({
...
async function handleResetPassword() {
  ...
  try {
    ...
    // Check whether the SM2 public key is configured
    ...
    try {
      // Concatenate the captcha and new password for encryption
    ...
    // Refetch the captcha after a reset failure
    refreshCaptcha()
  }
  finally {
    loading.value = false
...
// Fetch the captcha on page load
onLoad(() => {
  refreshCaptcha()
})

// Ensure the config is loaded when the component mounts
onMounted(async () => {
...
// Area code selector popup styles
.area-code-sheet {
  background: #ffffff;
