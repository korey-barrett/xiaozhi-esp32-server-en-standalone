      ...
      <div class="params-column">
        ...
        <div v-if="currentFunction" class="params-container">
          <el-form :model="currentFunction" class="param-form">
            <!-- Iterate over fieldsMeta rather than the keys of params -->
        ...
        <div class="mcp-left">
          ...
          <div class="url-header">
            <div class="address-desc">
              ...
              <a href="docs/mcp-endpoint-enable.md"
                target="_blank" class="doc-link">{{ $t('functionDialog.howToDeployMcp') }}</a> &nbsp;&nbsp;|&nbsp;&nbsp;
              <a href="docs/mcp-endpoint-integration.md"
...
export default {
  ...
  data() {
    return {
      ...
      modifiedFunctions: {},
      tempFunctions: {},
      // Add a flag to track whether saving has already been done
      hasSaved: false,
      loading: false,
  ...
  computed: {
    selectedList() {
      ...
      // If the address book feature is not enabled, filter out the device-to-device call plugin
  ...
  watch: {
    ...
    async value(v) {
      ...
      if (v) {
        ...
        // Initialize the selected state when the dialog opens
        this.selectedNames = this.functions.map(f => f.name);

        // If the address book feature is not enabled, remove the device-to-device call plugin from the selected list
        if (!this.featureStatus.addressBook) {
        ...
        // Merge the this.functions (with params) received from the backend onto allFunctions
        this.functions.forEach(saved => {
          const idx = this.allFunctions.findIndex(f => f.name === saved.name);
          if (idx >= 0) {
            // Keep the user's previous changes on saved.params
            this.allFunctions[idx].params = { ...saved.params };
          }
        });
        // Default the right side to the first one
  ...
  methods: {
    ...
    copyUrl() {
      ...
      textarea.style.position = 'fixed';  // Prevent the page from scrolling
    ...
    saveSelection() {
      ...
      // If the address book feature is not enabled, automatically unselect the device-to-device call plugin
      if (!this.featureStatus.addressBook) {
      ...
      this.dialogVisible = false;
      // Notify the parent component that the dialog has closed and saving is complete
