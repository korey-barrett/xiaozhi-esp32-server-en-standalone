...
export default {
  ...
  watch: {
    visible(val) {
      ...
      if (val) {
        ...
        // If it is a new knowledge base and ragModelId has not been set, default to selecting the first RAG model
    ...
    // Listen for changes to the RAG model list, ensuring the default value is correctly set when adding
    ragModels(newModels) {
  ...
  methods: {
    handleClose() {
      // Do not reset the form fields so that previous selections can be retained when editing
    ...
    loadRAGModels() {
      ...
      console.log('Starting to load the RAG model list');
      Api.model.getRAGModels((res) => {
        console.log('RAG model list response:', res);
        if (res.data && res.data.code === 0) {
          this.ragModels = res.data.data || [];
          console.log('RAG model list loaded successfully, total', this.ragModels.length, 'models');
          ...
          console.error('Failed to get the RAG model list:', res.data?.msg);
