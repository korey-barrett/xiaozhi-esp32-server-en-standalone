...
export default {
  ...
  methods: {
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          // If it is an array type, validate the format and convert
          if (submitData.valueType === 'array' && submitData.paramValue) {
            const lines = submitData.paramValue.split('\n').filter(line => line.trim());

            // Check that every line except the last ends with a semicolon
          ...
          // If it is a json type, compact the JSON format before submitting
    ...
    // Provided for the parent component to call to reset the saving state
    resetSaving() {
      this.saving = false;
    }
  },
    ...
    visible(newVal) {
      if (newVal) {
        if (this.form.paramValue) {
          // If it is a json type, format for display
          ...
          // If it is an array type, convert the semicolon-separated string to one item per line
        ...
        // When the dialog is closed, reset the saving state
        this.saving = false;
      }
    }
  }
