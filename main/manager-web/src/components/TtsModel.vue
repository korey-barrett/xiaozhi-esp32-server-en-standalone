...
export default {
  ...
  data() {
    return {
      ...
      selectedRows: [],
      loading: false,
      showReferenceColumns: false, // Controls whether the reference columns are shown
    };
  },
  watch: {
    visible(newVal) {
      ...
      if (newVal) {
        this.currentPage = 1;
        this.updateShowReferenceColumns(); // update the display state
        this.loadData(); // load data when the dialog is shown
        this.$nextTick(() => {
  ...
  methods: {
    // Update whether the reference columns are shown
    ...
    cancelEdit(row) {
      // For data created through "add new", when cancel editing, it needs to be removed from the array
      if (!row.id) {
    ...
    saveEdit(row) {
      ...
      try {
        ...
        // Only add the reference fields when the reference columns are shown
        ...
        let res;
        if (row.id) {
          // Has an ID, perform the update operation
          Api.timbre.updateVoice(params, (response) => {
            res = response;
            this.handleResponse(res, row);
          });
        } else {
          // No ID, perform the add operation
        ...
        // Also restore the original data on exception
        if (row.originalData) {
          Object.assign(row, row.originalData);
          row.editing = false;
          delete row.originalData;
    ...
    handleResponse(res, row) {
      if (res.code === 0) {
        ...
        // Restore the original data when saving fails
        if (row.originalData) {
          Object.assign(row, row.originalData);
          row.editing = false;
          delete row.originalData;
    ...
    addNew() {
      ...
      const newRow = {
        ...
        selected: false,
        editing: true,
        sort: 0 // new data defaults to being sorted at the top
      };
    ...
    deleteRow(row) {
      // Handle a single voice or an array of voices
      ...
      this.$confirm(this.$t('ttsModel.confirmDeleteVoice', {count: voiceCount}), this.$t('ttsModel.warning'), {
        ...
        Api.timbre.deleteVoice(ids, ({ data }) => {
          if (data.code === 0) {
            ...
            this.loadData(); // refresh the parameter list
          } else {
            this.$message.error({
...
/* Special handling for the audio input */
.audio-input ::v-deep .el-input__inner {
  min-width: 200px;
}

/* Flexible layout for operation buttons */
