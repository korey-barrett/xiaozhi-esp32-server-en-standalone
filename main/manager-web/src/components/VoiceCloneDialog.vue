...
export default {
    ...
    methods: {
        ...
        async handleFileChange(file) {
            ...
            // Enter step 2 first to ensure the DOM is rendered
            this.currentStep = 2;

            // Wait for the DOM to update before loading the audio
            await this.$nextTick();
        ...
        drawWaveform() {
            const canvas = this.$refs.waveformCanvas;
            if (!canvas) {
                console.error('Canvas not found while drawing the waveform');
                return;
            }
            ...
            this.waveformData.forEach((value, index) => {
                // Normalize and scale up, using 80% of the height
        ...
        handleTimeUpdate() {
            // Playback progress can be updated here
        },
        handleAudioEnded() {
            this.isPlaying = false;
        },
        ...
        async uploadAudio() {
            ...
            // Validate the audio duration (8-60 seconds)
            if (this.audioBuffer) {
                const duration = this.audioBuffer.duration;
