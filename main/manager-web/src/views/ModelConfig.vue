...
export default {
  ...
  updated() {
    // Re-apply the header translated text after the component updates
    this.updateSelectionHeaderText();
  },
  ...
  methods: {
    // Update the selection header translated text
    ...
    selectionCellClassName({ row, column, rowIndex, columnIndex }) {
      // Only set data-content on the header row
    ...
    handleMenuSelect(index) {
      ...
      this.pageSize = 10; // Optional: reset the page size
      this.loadData();
    },
    handleSearch() {
      this.currentPage = 1;
    ...
    handleModelSave({ provideCode, formData, done }) {
      ...
      if (this.editModelData.duplicateMode) {
        ...
        Api.model.addModel({ modelType, provideCode, formData }, ({ data }) => {
          ...
          done && done(); // Call the done callback to close the loading state
        });
      } else {
    ...
    // Handle enable/disable status changes
    handleStatusChange(model) {
      ...
      Api.model.updateModelStatus(model.id, newStatus, ({ data }) => {
        if (data.code === 0) {
          ...
          // Restore the original status when the operation fails
...
.main-wrapper {
  // 72px for the top 63px, bottom 35px, and search bar
...
::v-deep .el-table .custom-selection-header .cell::before {
  ...
  text-align: center;
  line-height: 32px;
  /* Set a suitable line height to ensure the text is fully displayed */
  color: black;
  margin-top: 0;
  /* Remove the top margin that may cause offset */
  height: 32px;
  position: absolute;
  top: 50%;
  left: 50%;
...
/* Removed the empty pseudo-element that may affect text display */
