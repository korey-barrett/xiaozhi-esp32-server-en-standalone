...
export default {
  ...
  methods: {
    // Return to the template management page
    goToHome() {
    ...
    prepareConfigData() {
      return {
        ...
        sort: this.form.sort,
        functions: [],
        // Include the required model fields to ensure the API call succeeds
        ...this.form.model
      };
    },
    ...
    // Get the template list and set the sort number
  ...
  // Perform initialization when the component is mounted
  mounted() {
    const templateId = this.$route.query.templateId;
    
    if (templateId) {
      // Edit mode: load the existing template
      this.fetchTemplateById(templateId);
    } else {
      // Create mode: set default values and get the sort number
