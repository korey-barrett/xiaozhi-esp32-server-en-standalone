...
export default {
  ...
  computed: {
    ...
    // Get the display text of the current language
    currentLanguageText() {
    ...
    // Get the corresponding xiaozhi-ai icon for the current language
    xiaozhiAiIcon() {
      const currentLang = this.currentLanguage;
      switch (currentLang) {
        case "zh_CN":
  ...
  methods: {
    ...
    fetchCaptcha() {
      // Handle the issue where manually clearing localStorage prevents fetching the captcha
    ...
    // Handle the visibility state change of the language dropdown menu
    ...
    getUserInfo() {
      Api.user.getUserInfo(({ data }) => {
        if (data.code === 0) {
          ...
          showDanger("Failed to fetch user info");
        }
      });
    },
