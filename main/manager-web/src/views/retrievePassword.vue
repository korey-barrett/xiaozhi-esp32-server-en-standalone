...
export default {
  ...
  computed: {
    ...
    // Get the xiaozhi-ai icon corresponding to the current language
    xiaozhiAiIcon() {
  ...
  methods: {
    ...
    // Reuse the captcha fetch method
    fetchCaptcha() {
    ...
    retrievePassword() {
      ...
      try {
        // Concatenate the graphic captcha and new password, then encrypt
      ...
      Api.user.retrievePassword({
        ...
        if (err.data != null && err.data.msg != null && (err.data.msg.indexOf('Graphic verification code') > -1 || err.data.msg.indexOf('Captcha') > -1)) {
          this.fetchCaptcha()
        }
      });
    },
