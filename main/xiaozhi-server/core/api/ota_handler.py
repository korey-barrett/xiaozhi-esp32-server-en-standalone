...
class OTAHandler(BaseHandler):
    ...
    def _get_websocket_url(self, local_ip: str, port: int) -> str:
        ...
        if "YOUR_" not in websocket_config:
            return websocket_config
        else:
            return f"ws://{local_ip}:{port}/xiaozhi/v1/"

    async def handle_post(self, request):
        """Handle OTA POST requests
        ...
        try:
            ...
            if mqtt_gateway_endpoint:  # If a non-empty string is configured
                # Try to get the device model from the request data (already parsed above)
                try:
                ...
                if signature_key:
                    ...
                    if not password:
                        password = ""  # Leave empty if signature fails; the device decides whether passwordless is allowed
                else:
                    self.logger.bind(tag=TAG).warning("Missing MQTT signature key, leaving password empty")

                # Build MQTT config (use the mqtt_gateway string directly)
                return_json["mqtt"] = {
                ...
                self.logger.bind(tag=TAG).info(f"Delivered MQTT gateway config to device {device_id}")
                ...
                self.logger.bind(tag=TAG).info(
                    f"No MQTT gateway configured, delivering WebSocket config to device {device_id}"
                )
            ...
            try:
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f"Delivered firmware {chosen_version} to device {device_id} [if the URL prefix is wrong, check server.vision_explain in the config file]-> {chosen_url} "
                    )
                else:
                    self.logger.bind(tag=TAG).info(
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            response = web.Response(text="OTA interface error", content_type="text/plain")
        finally:
            self._add_cors_headers(response)
            return response
    ...
    async def handle_download(self, request):
        ...
        - Only .bin files under the data/bin directory may be downloaded
