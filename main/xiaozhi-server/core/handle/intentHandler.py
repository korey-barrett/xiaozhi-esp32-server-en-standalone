...
async def handle_user_intent(conn: "ConnectionHandler", text):
    # Preprocess the input text, handling possible JSON format
    try:
        if text.strip().startswith("{") and text.strip().endswith("}"):
            parsed_data = json.loads(text)
            if isinstance(parsed_data, dict) and "content" in parsed_data:
                text = parsed_data["content"]  # Extract content for intent analysis
                conn.current_speaker = parsed_data.get("speaker")  # Preserve the speaker info
    except (json.JSONDecodeError, TypeError):
        pass

    # Check for a clear exit command
    ...
    if conn.intent_type == "function_call":
        ...
        return False
    # Use the LLM for intent analysis
    intent_result = await analyze_intent_with_llm(conn, text)
    if not intent_result:
        return False
    # Generate a sentence_id when the session starts
    conn.sentence_id = str(uuid.uuid4().hex)
    # Handle the various intents
...
async def check_direct_exit(conn: "ConnectionHandler", text):
    ...
    for cmd in cmd_exit:
        if text == cmd:
            conn.logger.bind(tag=TAG).info(f"Recognized a clear exit command: {text}")
...
async def analyze_intent_with_llm(conn: "ConnectionHandler", text):
    """Use the LLM to analyze user intent"""
...
async def process_intent_result(
    ...
    try:
        ...
        # Check for a function_call
        if "function_call" in intent_data:
            # Got a function_call directly from intent recognition
            ...
            if function_name == "result_for_context":
                ...
                def process_context_result():
                    ...
                    # Build the context-based base prompt
                    ...
                    # Use an async call to avoid blocking the event loop and affecting audio playback on other devices
                    try:
                        ...
                        conn.logger.bind(tag=TAG).error(f"LLM reply generation failed: {e}")
                        response = None
                    if response:
            ...
            if "arguments" in intent_data["function_call"]:
                ...
                if function_args is None:
                    function_args = {}
            # Ensure the arguments are in JSON string format
            ...
            # Use the executor to run the function call and result processing
        ...
        conn.logger.bind(tag=TAG).error(f"Error processing the intent result: {e}")
        return False


def speak_txt(conn: "ConnectionHandler", text):
    # Record the text-to-sentence_id mapping
