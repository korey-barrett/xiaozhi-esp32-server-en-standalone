...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        # If an internal address is configured (contains -internal.aliyuncs.com), use the ws protocol; otherwise default to wss
        if "-internal." in self.host:
            self.ws_url = f"ws://{self.host}/ws/v1"
        else:
            # Default to the wss protocol
        ...
        if self.access_key_id and self.access_key_secret:
            ...
            raise ValueError("Must provide access_key_id + access_key_secret or provide a token directly")
    ...
    async def receive_audio(self, conn, pcm_frame, audio_have_voice):
        ...
        # Establish connection only when there is voice and no connection exists (excluding stopping state)
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        self.server_ready = False  # Reset server readiness state
        ...
        logger.bind(tag=TAG).debug("Start request sent, waiting for server readiness...")
        ...
        try:
            while not conn.stop_event.is_set():
                ...
                try:
                    ...
                    if status != 20000000:
                        if status == 40010004:
                            logger.bind(tag=TAG).warning(f"Please close the connection only after the server finishes responding, status code: {status}")
                            break
                        if status in [40000004, 40010003]:  # Connection timeout or client disconnect
                            ...
                            break
                        elif status in [40270002, 40270003]:  # Audio problem
                    ...
                    # TranscriptionStarted means the server is ready to receive audio data
                    if message_name == "TranscriptionStarted":
                        ...
                        if text:
                            ...
                            if conn.client_listen_mode == "manual":
                                ...
                                # In manual mode, trigger processing only after receiving the stop signal (processed only once)
                    ...
                    logger.bind(tag=TAG).error("Timed out waiting for result")
                    break
