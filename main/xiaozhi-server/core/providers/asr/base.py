...
class ASRProviderBase(ABC):
    ...
    async def receive_audio(self, conn: "ConnectionHandler", pcm_frame, audio_have_voice):
        if conn.client_listen_mode == "manual":
            ...
            # If there is no voice, and there was no voice before, cache only part of the audio
            if not audio_have_voice and not conn.client_have_voice:
                conn.asr_audio = conn.asr_audio[-10:]
                return

            # In auto mode, trigger recognition when VAD detects voice stop
            if conn.asr.interface_type != InterfaceType.STREAM and conn.client_voice_stop:
                # Use the PCM data from asr_audio directly
                pcm_bytes = b"".join(conn.asr_audio)
                # Check whether there is enough audio data (each frame is 1920 bytes, 15 frames is about 28800 bytes)
    ...
    async def handle_voice_stop(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        ...
        try:
            ...
            # Data is already PCM, use it directly
            ...
            # Record recognition results - check whether it is an exception
            ...
            if isinstance(raw_text, dict):
                ...
                # Build a JSON string containing speaker information
    ...
    def _build_enhanced_text(self, text: str, speaker_name: Optional[str]) -> str:
        """Build text containing speaker information (for plain-text ASR only)"""
    ...
    def prefers_temp_file(self) -> bool:
        """Whether a temporary file is preferred"""
        return False
    ...
    async def speech_to_text(
        ...
        :param opus_data: The input Opus audio data
        :param session_id: The session ID
        :param artifacts: Audio artifacts, including PCM data, file paths, etc.
        :return: The recognition result text and file path (if any)
        """
        pass
