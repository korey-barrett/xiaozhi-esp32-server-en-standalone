...
class ASRProvider(ASRProviderBase):
    ...
    def _load_model(self):
        ...
        try:
            ...
            # Initialize the VOSK recognizer (the sample rate must be 16kHz)
    ...
    async def speech_to_text(
        ...
        try:
            ...
            if not self.model:
                logger.bind(tag=TAG).error("VOSK model is not loaded; recognition cannot proceed")
                return "", None
            ...
            # Perform recognition (VOSK recommends feeding 2000 bytes of data each time)
            chunk_size = 2000
            text_result = ""
            ...
            logger.bind(tag=TAG).debug(
                f"VOSK speech recognition elapsed: {time.time() - start_time:.3f}s | Result: {text_result.strip()}"
            )
