...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        self.session_conversation_map = {}  # Stores the mapping of session_id to conversation_id
            ...
            if self.mode == "chat-messages":
                for line in r.iter_lines():
                    if line.startswith(b"data: "):
                        ...
                        # If no conversation_id was found, fetch this conversation_id
                        if not conversation_id:
                            conversation_id = event.get("conversation_id")
                            self.session_conversation_map[session_id] = (
                                conversation_id  # Update the mapping
                            )
                        # Filter out the message_replace event, which pushes everything at once
    ...
    def response_with_functions(self, session_id, dialogue, functions=None):
        if len(dialogue) == 2 and functions is not None and len(functions) > 0:
            # First call to the LLM: take the last user message and append the tool prompt
        ...
        # If the last message has role="tool", append it to the user message
