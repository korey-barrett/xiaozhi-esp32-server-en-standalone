...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        # Check whether this is a qwen3 model
        self.is_qwen3 = self.model_name and self.model_name.lower().startswith("qwen3")

    def response(self, session_id, dialogue, **kwargs):
        # If this is a qwen3 model, prepend the /no_think directive to the user's last message
        if self.is_qwen3:
        ...
        is_active = True
        # Used to handle tags split across chunks
        buffer = ""
            ...
            for chunk in responses:
                try:
                    ...
                    if content:
                        ...
                        # Process the tags in the buffer
                        while "<think>" in buffer and "</think>" in buffer:
                            # Find the complete thinking/response tags and remove them
                            pre = buffer.split("<think>", 1)[0]
                            post = buffer.split("</think>", 1)[1]
                            buffer = pre + post

                        # Handle the case where only the opening tag is present
                        if "<think>" in buffer:
                            is_active = False
                            buffer = buffer.split("<think>", 1)[0]

                        # Handle the case where only the closing tag is present
                        if "</think>" in buffer:
                            is_active = True
                            buffer = buffer.split("</think>", 1)[1]

                        # Output if currently active and the buffer has content
