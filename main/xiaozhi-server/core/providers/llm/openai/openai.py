...
# Platform domains that require thinking mode to be disabled and their corresponding parameters (thinking mode is disabled by default)
...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        if isinstance(timeout_config, dict):
            ...
            # Backward compatibility with the old single timeout configuration (integer or float)
            custom_timeout = httpx.Timeout(timeout_config)
        else:
            # Not configured or invalid configuration, use the default value
    ...
    def normalize_dialogue(dialogue):
        """Automatically repair dialogue messages that are missing content"""
        for msg in dialogue:
    ...
    def response(self, session_id, dialogue, **kwargs):
        ...
        # Add optional parameters; only add when the parameter is not None
        optional_params = {
