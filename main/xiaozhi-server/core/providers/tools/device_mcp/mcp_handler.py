...
class MCPClient:
    ...
    def __init__(self):
        ...
        self.name_mapping = {}
        self.ready = False
        self.call_results = {}  # Stores Futures for tool call responses
        self.next_id = 1
...
async def handle_mcp_message(
    ...
    """Handle MCP messages, including initialization, tool list, and tool call responses, etc."""
    logger.bind(tag=TAG).debug(f"Processing MCP message: {str(payload)[:100]}")

    if not isinstance(payload, dict):
        logger.bind(tag=TAG).error("MCP message is missing the payload field or is malformed")
        return
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                if next_cursor:
                    ...
                    logger.bind(tag=TAG).debug("All tools fetched; MCP client is ready")

                    # Refresh the tool cache so MCP tools are included in the function list
...
async def send_mcp_initialize_message(conn: "ConnectionHandler"):
    ...
    # Generate a token from the secret key
...
async def send_mcp_tools_list_continue_request(conn: "ConnectionHandler", cursor: str):
    """Send an MCP tool list request with a cursor"""
    payload = {
        "jsonrpc": "2.0",
...
async def call_mcp_tool(
    ...
    """
    Call the specified tool and wait for its response.
    """
    ...
    logger.bind(tag=TAG).info(f"Sending client MCP tool call request: {actual_name}, arguments: {args}")
    await send_mcp_message(conn, payload)
