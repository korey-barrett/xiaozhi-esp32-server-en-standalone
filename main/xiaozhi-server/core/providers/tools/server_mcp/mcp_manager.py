...
class ServerMCPManager:
    """Centralized manager for multiple server-side MCP servers"""
    ...
    def __init__(self, conn) -> None:
        ...
        if not os.path.exists(self.config_path):
            ...
            logger.bind(tag=TAG).warning(
                f"Please check the MCP server configuration file: data/.mcp_server_settings.json"
            )
    ...
    async def _init_server(self, name: str, srv_config: Dict[str, Any]):
        ...
        try:
            ...
            # Use the lock to protect shared-state modifications
    ...
    def get_all_tools(self) -> List[Dict[str, Any]]:
        """Get the function definitions of all servers' tools"""
        return self.tools
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        """Execute a tool call, attempting to reconnect on failure"""
        logger.bind(tag=TAG).info(f"Executing server-side MCP tool {tool_name}, args: {arguments}")
    ...
    async def cleanup_all(self) -> None:
        ...
        for name, client in list(self.clients.items()):
            try:
                ...
                logger.bind(tag=TAG).info(f"Server-side MCP client shut down: {name}")
            except (asyncio.TimeoutError, Exception) as e:
                logger.bind(tag=TAG).error(f"Error shutting down server-side MCP client {name}: {e}")
        self.clients.clear()
