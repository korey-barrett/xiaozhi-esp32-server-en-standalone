...
class UnifiedToolHandler:
    ...
    async def _initialize(self):
        ...
        try:
            ...
            # Output the list of all currently supported tools
    ...
    async def _initialize_mcp_endpoint(self):
        ...
        try:
            ...
            # Get the MCP endpoint URL from the config
            ...
            if (
                ...
                if mcp_endpoint_client:
                    # Save the MCP endpoint client to the connection object
    ...
    def upload_functions_desc(self):
        ...
        self.logger.info("Function description list has been refreshed")
        ...
        try:
            ...
            if isinstance(arguments, str):
                try:
                    ...
                    self.logger.error(f"Failed to parse function arguments: {arguments}")
                    return ActionResponse(
                        action=Action.ERROR,
                        response="Failed to parse function arguments",
                    )
    ...
    def _combine_responses(self, responses: List[ActionResponse]) -> ActionResponse:
        """Merge the responses of multiple function calls"""
        if not responses:
            return ActionResponse(action=Action.NONE, response="No response")

        # If there is any error, return the first error
    ...
    async def cleanup(self):
        ...
        try:
            ...
            # Clean up the MCP endpoint connection
            if (
