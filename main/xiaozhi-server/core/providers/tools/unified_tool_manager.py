...
class ToolManager:
    ...
    def get_all_tools(self) -> Dict[str, ToolDefinition]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"Error getting tools of type {tool_type.value}: {e}")

        self._cached_tools = all_tools
        return all_tools

    def get_function_descriptions(self) -> List[Dict[str, Any]]:
        """Get the function descriptions of all tools (OpenAI format)"""
    ...
    def get_supported_tool_names(self) -> List[str]:
        """Get the names of all supported tools"""
        tools = self.get_all_tools()
    ...
    def refresh_tools(self):
        ...
        self._invalidate_cache()
        self.logger.debug("Tool cache has been refreshed")
    ...
    def get_tool_statistics(self) -> Dict[str, int]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"Error getting tool statistics for {tool_type.value}: {e}")
