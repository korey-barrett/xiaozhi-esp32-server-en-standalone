...
class TTSProvider(TTSProviderBase):
    ...
    def __init__(self, config, delete_audio_file):
        ...
        if self.access_key_id and self.access_key_secret:
            # Generate a temporary token using the key pair
            self._refresh_token()
        else:
            # Directly use the pre-generated long-term token
            self.token = config.get("token")
            self.expire_time = None

    def _refresh_token(self):
        """Refresh the token and record the expiry time"""
        if self.access_key_id and self.access_key_secret:
            self.token, expire_time_str = AccessToken.create_token(
                self.access_key_id, self.access_key_secret
            )
            if not expire_time_str:
                raise ValueError("Unable to obtain a valid token expiry time")

            try:
                # Normalize to string handling
    ...
    async def text_to_speak(self, text, output_file):
        ...
        try:
            ...
            # Check whether the returned mime type is audio/***; if so, save it to the given path (response is binary format)
