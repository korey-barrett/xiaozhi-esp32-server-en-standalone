...
class AccessToken:
    ...
    def create_token(access_key_id, access_key_secret):
        parameters = {
            ...
            "Action": "CreateToken",
            "Format": "JSON",
            "RegionId": "cn-shanghai",  # Use the Shanghai region to obtain the Token
...
class TTSProvider(TTSProviderBase):
    ...
    def __init__(self, config, delete_audio_file):
        ...
        if config.get("private_voice"):
            ...
            self.voice = config.get("voice", "longxiaochun")  # CosyVoice default timbre
        ...
        # Apply percentage adjustments (if present), otherwise use the public configuration
        self._apply_percentage_params(config)

        # WebSocket configuration
        self.host = config.get("host", "nls-gateway-cn-beijing.aliyuncs.com")
        # If an internal address is configured (contains -internal.aliyuncs.com), use the ws protocol; the default is wss
    ...
    def _refresh_token(self):
        ...
        if self.access_key_id and self.access_key_secret:
            ...
            if not expire_time_str:
                raise ValueError("Could not obtain a valid Token expiration time")
    ...
    async def _ensure_connection(self):
        ...
        try:
            ...
            if self.ws and current_time - self.last_active_time < 10:
                # The link can only be reused within 10 seconds for continuous conversation
                self.task_id = uuid.uuid4().hex
                logger.bind(tag=TAG).debug(f"Reusing existing link..., task_id: {self.task_id}")
                return self.ws
    ...
    async def text_to_speak(self, text, _):
        try:
            if self.ws is None:
                logger.bind(tag=TAG).warning("WebSocket connection does not exist, aborting text sending")
                return
            filtered_text = MarkdownCleaner.clean_markdown(text)

            if filtered_text:
                # Use sliding-window matching to handle replacement words split across chunks
    ...
    async def start_session(self, task_id):
        ...
        try:
            # If the previous session is still active, close the old connection and create a new link
                ...
                try:
                    ...
                    if isinstance(msg, str):  # Text control message
                        try:
                            ...
                            # Only handle responses from the currently active session
                            if task_id and self.task_id != task_id:
                                if event_name in ["SynthesisCompleted", "TaskFailed"]:
                                    logger.bind(tag=TAG).debug("Received residual downlink end response, resetting session state~~")
                                    self.activate_session = False
                                continue
                            ...
                            if event_name == "SynthesisStarted":
                                ...
                                logger.bind(tag=TAG).debug("Session ended~~")
                                self.activate_session = False
                    ...
                    break
            # Close the WebSocket on abnormal connection
            if self.ws:
                try:
                    await self.ws.close()
                except:
                    pass
                self.ws = None
        # Clean up references when the monitor task exits
        finally:
            self.activate_session = False
            self._monitor_task = None
    ...
    def audio_to_opus_data_stream(
        ...
        """Override the parent method: use a separate temporary encoder to process audio files,
        avoiding concurrent conflicts with the TTS streaming encoder.
        In dual-stream TTS, the monitor task receives TTS audio on the event loop thread and encodes
        it with self.opus_encoder, while tts_text_priority_thread also uses self.opus_encoder to process
        music files. The shared encoder.buffer is not thread-safe, and concurrent access causes the
    ...
    def to_tts(self, text: str) -> list:
        ...
        try:
            ...
            async def _generate_audio():
                ...
                try:
                    # Send the StartSynthesis request
                    start_request = {
                        "header": {
                            "message_id": uuid.uuid4().hex,
                            "task_id": self.task_id,
                    ...
                    # Wait for the SynthesisStarted response
                    ...
                    # Send the text synthesis request
                    filtered_text = MarkdownCleaner.clean_markdown(text)
                    ...
                    # Send the stop synthesis request
                    stop_request = {
                        "header": {
                            "message_id": uuid.uuid4().hex,
                            "task_id": self.task_id,
