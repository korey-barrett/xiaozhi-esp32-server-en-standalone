...
class TTSProviderBase(ABC):
    def __init__(self, config, delete_audio_file):
        ...
        # Mapping from sentence_id to text, used to retrieve the correct subtitle text for streaming TTS
        ...
        # Build the regex using longest-match-first (sort then escape and join)
        if self.correct_words:
            # Sort keys by length descending so longer words match first, avoiding short-word partial interference
            ...
            # Build the reverse replacement regex to convert the TTS-replaced text back to the original (subtitle display)
            ...
            for key in sorted_keys:  # Use keys already sorted by length descending to ensure longer words match first
        ...
        # Streaming sliding window: cached text awaiting matching
    ...
    def handle_opus(self, opus_data: bytes):
        logger.bind(tag=TAG).debug(f"Pushing data into the queue frames~~ {len(opus_data)}")
    ...
    def to_tts_stream(self, text, opus_handler: Callable[[bytes], None] = None) -> None:
        # Keep the original text for display/reporting
        original_text = text
        text = MarkdownCleaner.clean_markdown(text)
        # Use regex to replace all at once, avoiding repeated iteration and partial-match issues
        if self._correct_words_pattern:
            text = self._correct_words_pattern.sub(lambda m: self.correct_words[m.group(0)], text)
        max_repeat_time = 5
        if self.delete_audio_file:
            # If the file needs to be deleted, convert directly to audio data
            while max_repeat_time > 0:
                try:
                    audio_bytes = asyncio.run(self.text_to_speak(text, None))
                    if audio_bytes:
                        # Use the original text for display/reporting
            ...
            if max_repeat_time > 0:
                logger.bind(tag=TAG).info(
                    f"Speech generated successfully: {original_text}, retried {5 - max_repeat_time} times"
                )
            else:
                logger.bind(tag=TAG).error(
                    f"Speech generation failed: {original_text}, please check the network or service"
                )
            return None
        else:
            ...
            try:
                while not os.path.exists(tmp_file) and max_repeat_time > 0:
                    try:
                        ...
                        # Deletion failed; remove the file
    ...
    def to_tts(self, text):
        # Keep the original text for logging/display
        original_text = text
        text = MarkdownCleaner.clean_markdown(text)
        if self._correct_words_pattern:
            text = self._correct_words_pattern.sub(lambda m: self.correct_words[m.group(0)], text)
    ...
    def audio_to_pcm_data_stream(
        ...
        """Convert an audio file to PCM encoding"""
    ...
    def audio_to_opus_data_stream(
        ...
        """Convert an audio file to Opus encoding"""
    ...
    def tts_one_sentence(
        ...
        # Segment a single-sentence text into chunks
        segments = re.split(r"([。！？!?；;\n])", content_detail)
        for seg in segments:
    ...
    async def open_audio_channels(self, conn):
        ...
        # Create an encoder based on conn's sample_rate; do not overwrite it if a subclass has already created one (IndexTTS interface returns 24kHz - pending resampling)
    ...
    def store_tts_text(self, sentence_id, text):
        """Store the text corresponding to the given sentence_id, used to retrieve the correct subtitle text for streaming TTS

        Args:
            sentence_id: the session ID
    ...
    def get_tts_text(self, sentence_id):
        """Get the text corresponding to the given sentence_id
        ...
        while not self.conn.stop_event.is_set():
            try:
                ...
                if self.conn.client_abort:
                    logger.bind(tag=TAG).info("Received interrupt message, terminating TTS text processing thread")
                    continue
                # Filter old messages: check whether sentence_id matches
                ...
                logger.bind(tag=TAG).error(
                    f"Failed to process TTS text: {str(e)}, type: {type(e).__name__}, trace: {traceback.format_exc()}"
                )
                continue
    ...
    def _audio_play_priority_thread(self):
        ...
        while not self.conn.stop_event.is_set():
            ...
            try:
                ...
                # Report when the next text begins or the session ends
                if sentence_type is not SentenceType.MIDDLE:
                    if self.report_on_last:
                        # Accumulation mode: for TTS with a single continuous speech stream (e.g. seed-tts-2.0)
                        # At FIRST only record the text; audio keeps accumulating and is reported only once at LAST
                        if text:
                            enqueue_text = text
                ...
                # Collect the reported audio data
    ...
    def _get_segment_text(self):
        # Merge all current text and process the unsegmented part
        full_text = "".join(self.tts_text_buff)
        current_text = full_text[self.processed_chars :]  # Start from the unprocessed position
        last_punct_pos = -1

        # Choose a different set of punctuation based on whether this is the first sentence
        punctuations_to_use = (
            self.first_sentence_punctuations
        ...
        if last_punct_pos != -1:
            ...
            self.processed_chars += len(segment_text_raw)  # Update the processed character position

            # If this is the first sentence, set the flag to False once the first comma is found
            if self.is_first_sentence:
    ...
    def _process_audio_file_stream(
        ...
        """Process an audio file and convert it to the specified format

        Args:
            tts_file: the audio file path
            callback: the file processing function
        """
    ...
    def _apply_percentage_params(self, config):
        """Apply percentage parameters in bulk according to the TTS_PARAM_CONFIG defined in the subclass"""
    ...
    def _match_stream_text(self, text):
        """Streaming text sliding-window matching, used to handle replacement words split across chunks

        Args:
            text: the input text segment

        Returns:
            tuple: (list of confirmed texts, remaining prefix awaiting match)
        """
        ...
        while i < len(text):
            ...
            # No longer word matched; the pending content is now confirmed to be sent
            if pending:
                result.append(pending)
                pending = ""

            # Check whether the current char is the start of a replacement word
    ...
    def reset_stream_state(self):
        """Reset the streaming processing state, used to clean up residual state when a session starts"""
        self._pending_prefix = ""
