...
EVENT_ConnectionFailed = 51  # Connection failed (possibly unable to pass permission authentication)
...
class TTSProvider(TTSProviderBase):
    ...
    async def _ensure_connection(self):
        ...
        try:
            ...
            # After the connection is established, start the monitor task
    ...
    def tts_text_priority_thread(self):
        """Text processing thread for Volcano Engine dual-stream TTS"""
        while not self.conn.stop_event.is_set():
            try:
                ...
                if self.conn.client_abort:
                    try:
                        logger.bind(tag=TAG).info("Received interruption info, terminating the TTS text processing thread")
                        if self.enable_ws_reuse:
                        ...
                        logger.bind(tag=TAG).error(f"Canceling TTS session failed: {str(e)}")
                        continue
                ...
                if message.sentence_type == SentenceType.FIRST:
                    ...
                    try:
                        ...
                        logger.bind(tag=TAG).error(f"Starting TTS session failed: {str(e)}")
                        continue
                    ...
                    logger.bind(tag=TAG).info(
                        f"Adding audio file to pending play list: {message.content_file}"
                    )
    ...
    async def text_to_speak(self, text, _):
        ...
        try:
            ...
            if self.ws is None:
                logger.bind(tag=TAG).warning(f"WebSocket connection does not exist, aborting text sending")
                return
    ...
    async def start_session(self, session_id):
        ...
        try:       
            ...
            logger.bind(tag=TAG).error(f"Starting session failed: {str(e)}")
    ...
    async def finish_session(self, session_id):
        ...
        try:
            ...
            logger.bind(tag=TAG).error(f"Closing session failed: {str(e)}")
            # Ensure resource cleanup
            await self.close()
            raise

    async def cancel_session(self,session_id):
        logger.bind(tag=TAG).debug(f"Canceling session, releasing server resources ~~{session_id}")
        try:
            if self.ws:
                header = Header(
                    message_type=FULL_CLIENT_REQUEST,
            ...
            logger.bind(tag=TAG).error(f"Canceling session failed: {str(e)}")
            # Ensure resource cleanup
            await self.close()
            raise
        ...
        try:
            while not self.conn.stop_event.is_set():
                try:
                    ...
                    # Only process responses of the current active session
                    if res.optional.sessionId and self.conn.sentence_id != res.optional.sessionId:
                        # If it is a session-end related event, reset state even if the session ID does not match
                    ...
                    if res.optional.event == EVENT_SessionCanceled:
                        ...
                        # Restore the replaced text returned by the TTS service to the original text for subtitle display
                    ...
                    traceback.print_exc()
                    break
            # Close the WebSocket on connection exception
            if self.ws:
                try:
                    await self.ws.close()
                except:
    ...
    def get_payload_bytes(
        ...
        # If mix_speaker configuration exists, add it to req_params
        if self.mix_speaker:
    ...
    def audio_to_opus_data_stream(
        ...
        """Override the parent method: use an independent temporary encoder to process audio files,
        avoiding concurrency conflicts with the TTS streaming encoder.
        In dual-stream TTS, the monitor task receives TTS audio on the event-loop thread and encodes it
        using self.opus_encoder, while tts_text_priority_thread also uses self.opus_encoder to process music files.
        The shared encoder.buffer is not thread-safe, and concurrent access causes the SILK resampler assertion to fail.
        """
    ...
    async def _cancel_monitor_task(self):
        ...
        if self._monitor_task and not self._monitor_task.done():
            ...
            try:
                ...
                logger.bind(tag=TAG).warning(f"Error canceling the monitor task: {e}")
        self._monitor_task = None

    def to_tts(self, text: str) -> list:
        """Generate audio data non-streamingly, for audio generation and testing scenarios
        Args:
