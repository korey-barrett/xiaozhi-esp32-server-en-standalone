...
class TTSProvider(TTSProviderBase):
    ...
    async def open_audio_channels(self, conn):
        ...
        # Update the sample rate in audio_setting to the actual conn.sample_rate
        ...
        try:
            ...
            try:
                ...
                logger.bind(tag=TAG).warning(
                    f"Speech generation failed {5 - max_repeat_time + 1} time(s): {original_text}, error: {e}"
                )
                max_repeat_time -= 1

            if max_repeat_time > 0:
                logger.bind(tag=TAG).info(
                    f"Speech generated successfully: {original_text}, retried {5 - max_repeat_time} time(s)"
                )
            else:
                logger.bind(tag=TAG).error(
                    f"Speech generation failed: {original_text}, please check whether the network or service is working properly"
                )
        except Exception as e:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    async for chunk in resp.content.iter_any():
                        ...
                        while True:
                            ...
                            try:
                                ...
                                # Check for business-layer errors
                                ...
                                # Only process valid audio blocks with status=1; ignore the end summary block with status=2
                    ...
                    # Flush remaining data that is less than one frame
                    if self.pcm_buffer:
                        self.opus_encoder.encode_pcm_to_opus_stream(
                    ...
                    # If this is the last segment, audio acquisition is complete
                    if is_last:
                        self._process_before_stop_play_files()
        ...
        try:
            with requests.post(
                ...
                for data_block in full_content.split('\n\n'):
                    ...
                    try:
                        json_str = data_block[6:]  # Remove the 'data: ' prefix
                        data = json.loads(json_str)
                ...
                # Process the merged PCM data in frames
