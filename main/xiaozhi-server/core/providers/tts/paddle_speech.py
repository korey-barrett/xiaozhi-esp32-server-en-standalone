...
class TTSProvider(TTSProviderBase):
    ...
    async def pcm_to_wav(self, pcm_data: bytes, sample_rate: int = 24000, num_channels: int = 1,
        ...
        """
        Convert PCM data to a WAV file and return the byte data
        ...
        :param bits_per_sample: Number of bits per sample, default 16
    ...
    async def text_streaming(self, text, output_file):
        try:
            # Use websockets to asynchronously connect to the WebSocket server
            async with websockets.connect(self.url) as ws:
                ...
                try:
                    ...
                    raise Exception(f"WebSocket timeout: waited for audio data for more than {timeout_seconds} seconds")
                ...
                # Receive end response to avoid the service throwing an exception
                await ws.recv()

                # Decide whether to save the file based on configuration
