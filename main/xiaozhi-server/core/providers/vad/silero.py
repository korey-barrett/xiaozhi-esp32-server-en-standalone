...
class VADProvider(VADProviderBase):
    ...
    def release_conn_resources(self, conn):
        """Release the connection's VAD resources (called when the connection closes)"""
    ...
    def is_vad(self, conn, pcm_frame):
        # Manual mode: return True directly, no real-time VAD detection, all audio is cached
        ...
        try:
            ...
            while len(conn.client_audio_buffer) >= 512 * 2:
                ...
                # If the speech did not drop below the low threshold, continue the previous state, judged as having voice
                ...
                # If there was voice before but not this time, and the time since the last voice exceeds the silence threshold, consider the sentence finished
