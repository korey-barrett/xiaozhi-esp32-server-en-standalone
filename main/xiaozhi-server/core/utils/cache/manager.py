...
class GlobalCacheManager:
    ...
    def set(
        ...
        # Use the config TTL or the passed-in TTL
        ...
        with self._locks[cache_name]:
            ...
            if config.strategy in [CacheStrategy.LRU, CacheStrategy.TTL_LRU]:
                # LRU strategy: move the existing entry to the end
                if key in cache:
                    del cache[key]
                cache[key] = entry
                ...
                # Check the size limit
                if config.max_size and len(cache) > config.max_size:
                    # Simple strategy: remove one entry at random
