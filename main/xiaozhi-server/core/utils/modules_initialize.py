...
def initialize_voiceprint(asr_instance, config):
    ...
    try:
        ...
        logger.bind(tag=TAG).info("ASR module voiceprint recognition feature dynamically enabled")
        logger.bind(tag=TAG).info(f"Configured number of speakers: {len(voiceprint_config['speakers'])}")
        return True
