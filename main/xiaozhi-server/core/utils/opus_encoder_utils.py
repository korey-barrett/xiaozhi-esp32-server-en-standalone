...
class OpusEncoderUtils:
    ...
    def __init__(self, sample_rate: int, channels: int, frame_size_ms: int):
        ...
        # Compute samples per frame = sample rate * frame size (ms) / 1000
    ...
    def encode_pcm_to_opus_stream(self, pcm_data: bytes, end_of_stream: bool, callback: Callable[[Any], Any]):
        """
        Encode PCM data into Opus format, processed in a streaming manner
        ...
        # Handle remaining data at the end of the stream
        if end_of_stream and len(self.buffer) > 0:
            # Create the last frame and pad it with zeros
    ...
    def _encode(self, frame: np.ndarray) -> Optional[bytes]:
        ...
        try:
            # The encoder has been released, skip encoding
            if not hasattr(self, 'encoder') or self.encoder is None:
                return None
            # Convert the numpy array to bytes
            frame_bytes = frame.tobytes()
            # opuslib requires that the input byte count be a multiple of channels*2
    ...
    def _convert_bytes_to_shorts(self, bytes_data: bytes) -> np.ndarray:
        """Convert a byte array to a short array (16-bit PCM)"""
    ...
    def _validate_pcm_data(self, pcm_shorts: np.ndarray) -> None:
        """Validate whether the PCM data is valid"""
        ...
        if np.any((pcm_shorts < -32768) | (pcm_shorts > 32767)):
            ...
            # In real applications you could clip instead of raising an exception
