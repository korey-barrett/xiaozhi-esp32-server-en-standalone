...
class VoiceprintProvider:
    ...
    def __init__(self, config: dict):
        ...
        # Parse the API address and key
        self.api_url = None
        self.api_key = None
        self.speaker_ids = []
        
        if not self.original_url:
            logger.bind(tag=TAG).warning("Voiceprint recognition URL is not configured, voiceprint recognition will be disabled")
            self.enabled = False
        else:
            # Parse the URL and key
            ...
            if not self.api_key:
                logger.bind(tag=TAG).error("No key parameter found in the URL, voiceprint recognition will be disabled")
                self.enabled = False
            else:
                # Build the identify API address
                self.api_url = f"{base_url}/voiceprint/identify"
                
                # Extract the speaker_ids
    ...
    def _check_server_health(self) -> bool:
        """Check the voiceprint recognition server health status"""
        ...
        try:
            ...
            logger.bind(tag=TAG).warning("Voiceprint recognition server connection was refused")
            is_healthy = False
        except Exception as e:
            logger.bind(tag=TAG).warning(f"Voiceprint recognition server health check error: {e}")
            is_healthy = False
        
        # Use the global cache manager to cache the result
    ...
    async def identify_speaker(self, audio_data: bytes, session_id: str) -> Optional[str]:
        ...
        if not self.enabled or not self.api_url or not self.api_key:
            logger.bind(tag=TAG).debug("Voiceprint recognition is disabled or not configured, skipping recognition")
            return None
        ...
        try:
            ...
            # Prepare the multipart/form-data data
                ...
                async with session.post(self.api_url, headers=headers, data=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        if score < self.similarity_threshold:
                            logger.bind(tag=TAG).warning(f"Voiceprint recognition similarity {score:.3f} is below threshold {self.similarity_threshold}")
