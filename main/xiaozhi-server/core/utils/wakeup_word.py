        ...
        while True:
            try:
                ...
                if time.time() - self.start_time > self.timeout:
                    raise TimeoutError("Timed out acquiring the file lock")
                time.sleep(0.1)
...
class WakeupWordsConfig:
    ...
    def _load_config(self) -> Dict:
        """Load the config file, using a caching mechanism"""
        current_time = time.time()

        # If the cache is valid, return the cache directly
        if (
        ...
        try:
            ...
            print(f"Unknown error while loading the config file: {e}")
            return {}

    def _save_config(self, config: Dict):
        """Save the config to the file, protected by a file lock"""
        try:
            ...
            print(f"Unknown error while saving the config file: {e}")
            raise

    def get_wakeup_response(self, voice: str) -> Dict:
        voice = hashlib.md5(voice.encode()).hexdigest()
        """Get the wakeup word reply config"""
    ...
    def update_wakeup_response(self, voice: str, file_path: str, text: str):
        """Update the wakeup word reply config"""
        try:
            # Filter out emoji
            ...
            print(f"Failed to update the wakeup word reply config: {e}")
            raise

    def generate_file_path(self, voice: str) -> str:
        """Generate the audio file path, using the hash of the voice as the filename"""
        try:
            # Generate the hash of the voice
            voice_hash = hashlib.md5(voice.encode()).hexdigest()
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    print(f"Failed to delete the existing audio file: {e}")
                    raise

            return file_path
        except Exception as e:
            print(f"Failed to generate the audio file path: {e}")
            raise