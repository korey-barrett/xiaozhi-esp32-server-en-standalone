...
class SuppressInvalidHandshakeFilter(logging.Filter):
    """Filter out invalid handshake error logs (e.g. HTTPS accessing a WS port)"""
...
def _setup_websockets_logger():
    """Configure all websockets-related loggers, filtering invalid handshake errors"""
...
class WebSocketServer:
    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        if headers.get("device-id", None) is None:
            ...
            if "device-id" not in query_params:
                await websocket.send("Port is running normally. To test the connection, start the digital-human test.")
                await websocket.close()
                return
            else:
        ...
        """Handle a new connection, creating an independent ConnectionHandler each time."""
    ...
    async def update_config(self) -> bool:
        """Update the server config and reinitialize components.
        ...
        try:
            async with self.config_lock:
                ...
                if new_config is None:
                    ...
                    return False
                self.logger.bind(tag=TAG).info(f"Successfully fetched new config")
    ...
    async def _handle_auth(self, websocket: websockets.ServerConnection):
        ...
        if self.auth_enable:
            ...
            if self.allowed_devices and device_id in self.allowed_devices:
                # If the device is in the whitelist, skip token verification and allow it directly
                return
            else:
