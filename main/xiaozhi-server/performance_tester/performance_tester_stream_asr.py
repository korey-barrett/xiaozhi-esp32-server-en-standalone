...
description = "Streaming ASR first-token latency test"
try:
    import dashscope
except ImportError:
    dashscope = None
...
class BaseASRTester:
    ...
    def _calculate_result(self, service_name, latencies, test_count):
        """Calculate test results (fix: correctly handle None values and exclude failed tests)"""
...
class DoubaoStreamASRTester(BaseASRTester):
    ...
    async def test(self, test_count=5):
        if not self.test_audio_files:
            return {"name": "Doubao Streaming ASR", "latency": 0, "status": "Failed: no test audio found"}
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(
                    ...
                    # Send the audio data (using the last-frame marker to tell the server the audio has ended)
                    ...
                    print(f"[Doubao ASR] Test {i+1} first-token latency: {latency:.3f}s")
                    await ws.close()
...
class QwenASRFlashTester(BaseASRTester):
    ...
    async def _test_single(self, audio_file_info):
        ...
        try:
            ...
            # Optimization: prepare the temporary file before timing to reduce the impact of disk I/O on the performance test
            ...
            if dashscope is None:
                raise RuntimeError("The dashscope library is not installed")

            dashscope.api_key = api_key

            # Unified timing start: start timing before the API call (but file preparation is complete)
            start_time = time.time()
            ...
            raise Exception("Streaming ended, no response was received")
    ...
    async def test(self, test_count=5):
        if not self.test_audio_files:
            return {"name": "Qwen ASR", "latency": 0, "status": "Failed: no test audio found"}
        if not self.asr_config and not os.getenv("DASHSCOPE_API_KEY"):
            return {"name": "Qwen ASR", "latency": 0, "status": "Failed: api_key not configured"}
        ...
        for i in range(test_count):
            try:
                ...
                latencies.append(latency)
                print(f"[Tongyi ASR] Test {i+1} first-token latency: {latency:.3f}s")
            except Exception as e:
...
class XunfeiStreamASRTester(BaseASRTester):
    ...
    async def test(self, test_count: int = 5):
        if not self.test_audio_files:
            return {"name": "Xunfei Streaming ASR", "latency": 0, "status": "Failed: no test audio found"}
        if not self.asr_config:
            return {"name": "Xunfei Streaming ASR", "latency": 0, "status": "Failed: not configured"}

        required = ["app_id", "api_key", "api_secret"]
        for k in required:
            if k not in self.asr_config:
                return {"name": "Xunfei Streaming ASR", "latency": 0, "status": f"Failed: missing config {k}"}
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(
                    ...
                    # First frame: remove the punc field to avoid an unknown-parameter error
                    ...
                    async for message in ws:
                        ...
                        if ws_result:
                            ...
                            if text.strip() and first_token:
                                latency = time.time() - start_time
                                latencies.append(latency)
                                print(f"[Xunfei ASR] Test {i+1} first-token latency: {latency:.3f}s")
                                first_token = False
                                break

            except Exception as e:
                print(f"[Xunfei ASR] Test {i+1} failed: {str(e)}")
                latencies.append(None)
...
class ASRPerformanceSuite:
    ...
    def _print_results(self, test_count):
        ...
        print(f"\n{'='*60}")
        print("Streaming ASR First-Token Response Time Test Results")
        print(f"{'='*60}")
        print(f"Test count: each ASR service is tested {test_count} times")
        ...
        print(tabulate(table_data, headers=["ASR Service", "First-Token Latency", "Status"], tablefmt="grid"))
        print("\nTest Notes:")
        print("- Timing start: before the connection is established (includes handshake, sending audio, and receiving the first recognition result)")
        print("- Qwen optimization: temporary file preparation completes before timing to reduce the impact of disk I/O on the test")
        print("- Error handling: failed tests are not included in the average; only successful test latencies are counted")
        print("- Sort rule: successful ones sorted by latency ascending, failed ones last")

    async def run(self, test_count=5):
        print(f"Starting streaming ASR first-token response time test...")
        print(f"Test count per ASR service: {test_count} times\n")
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(description="Streaming ASR first-token response time test tool")
    parser.add_argument("--count", type=int, default=5, help="number of test iterations")
