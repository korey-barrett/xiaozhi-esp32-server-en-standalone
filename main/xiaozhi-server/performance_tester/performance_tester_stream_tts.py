...
description = "Streaming TTS voice synthesis first-token time test"
class StreamTTSPerformanceTester:
    ...
    async def test_aliyun_tts(self, text=None, test_count=5):
        """Test Aliyun streaming TTS first-token latency (average over multiple runs)"""
        ...
        for i in range(test_count):
            try:
                ...
                # Unified timing start: start timing before the connection is established
                ...
                async with websockets.connect(ws_url, extra_headers={"X-NLS-Token": token}) as ws:
                    ...
                    while True:
                        ...
                        if isinstance(response, bytes):
                            ...
                            print(f"[Aliyun TTS] Test {i+1} first-token latency: {latency:.3f}s")
                            break
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(
                    ...
                    # 2. Wait for the task-started event (critical! must wait for this before sending text)
                    task_started = False
                    while not task_started:
                        ...
                        if isinstance(msg, str):
                            ...
                            if event == "task-started":
                                task_started = True
                                print(f"[Aliyun Bailian TTS] Test {i+1} task started successfully")
                    ...
                    # 5. Wait for the first audio data chunk
                    while True:
                        msg = await asyncio.wait_for(ws.recv(), timeout=15.0)
                        if isinstance(msg, (bytes, bytearray)) and len(msg) > 0:
                            latency = time.time() - start_time
                            print(f"[Aliyun Bailian TTS] Test {i+1} first-token latency: {latency:.3f}s")
                            latencies.append(latency)
                            break
                            ...
                            if event == "task-failed":
                                ...
                                if not latencies or latencies[-1] is None:
                                    raise Exception("Task finished but no audio was received")

            except Exception as e:
                print(f"[Aliyun Bailian TTS] Test {i+1} failed: {str(e)}")
                latencies.append(None)
    ...
    async def test_doubao_tts(self, text=None, test_count=5):
        """Test Volcano Engine streaming TTS first-token latency (average over multiple runs)"""
        text = text or self.test_texts[0]
        latencies = []
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(ws_url, additional_headers=ws_header, max_size=1000000000) as ws:
                    ...
                    latency = time.time() - start_time
                    latencies.append(latency)
                    print(f"[Volcano Engine TTS] Test {i+1} first-token latency: {latency:.3f}s")

            except Exception as e:
                print(f"[Volcano Engine TTS] Test {i+1} failed: {str(e)}")
                latencies.append(None)
    ...
    async def test_paddlespeech_tts(self, text=None, test_count=5):
        """Test PaddleSpeech streaming TTS first-token latency (average over multiple runs)"""
        text = text or self.test_texts[0]
        latencies = []
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(tts_url) as ws:
                    ...
                    latency = time.time() - start_time
                    latencies.append(latency)
                    print(f"[PaddleSpeechTTS] Test {i+1} first-token latency: {latency:.3f}s")
                ...
                print(f"[PaddleSpeechTTS] Test {i+1} failed: {str(e)}")
                latencies.append(None)
    ...
    async def test_indexstream_tts(self, text=None, test_count=5):
        """Test IndexStream streaming TTS first-token latency (average over multiple runs)"""
        text = text or self.test_texts[0]
        latencies = []
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(api_url, json=payload, timeout=10) as resp:
                        ...
                        async for chunk in resp.content.iter_any():
                            ...
                            latency = time.time() - start_time
                            latencies.append(latency)
                            print(f"[IndexStreamTTS] Test {i+1} first-token latency: {latency:.3f}s")
                            resp.close()
                            break
                        else:
                            latencies.append(None)
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(api_url, params=params, headers=headers, timeout=10) as resp:
                        ...
                        async for _ in resp.content.iter_any():
                            latency = time.time() - start_time
                            latencies.append(latency)
                            print(f"[LinkeraiTTS] Test {i+1} first-token latency: {latency:.3f}s")
                            break
                        else:
                            latencies.append(None)

            except Exception as e:
                print(f"[LinkeraiTTS] Test {i+1} failed: {str(e)}")
                latencies.append(None)
        
        return self._calculate_result("LinkeraiTTS", latencies, test_count)
    
    async def test_xunfei_tts(self, text=None, test_count=5):
        """Test Xunfei streaming TTS first-token latency (average over multiple runs)"""
        text = text or self.test_texts[0]
        latencies = []
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(
                    ...
                    # Wait for the first audio data chunk
                    ...
                    while not first_audio_received:
                        ...
                        if audio_payload:
                            ...
                            if status == 1 and audio_data:
                                # Received the first audio data chunk
                                latency = time.time() - start_time
                                latencies.append(latency)
                                print(f"[Xunfei TTS] Test {i+1} first-token latency: {latency:.3f}s")
                                first_audio_received = True
                                break
            except Exception as e:
                print(f"[Xunfei TTS] Test {i+1} failed: {str(e)}")
                latencies.append(None)
    ...
    def _calculate_result(self, service_name, latencies, test_count):
        """Calculate test results (correctly handle None values and exclude failed tests)"""
        # Exclude failed tests (None values and <=0 latencies); only count valid latencies
    ...
    def _print_results(self, test_text, test_count):
        ...
        print(f"\n{'='*60}")
        print("Streaming TTS First-Token Latency Test Results")
        print(f"{'='*60}")
        print(f"Test text: {test_text}")
        print(f"Test count: each TTS service is tested {test_count} times")

        # Sort the results: successful first, then by latency ascending
        ...
        print(tabulate(table_data, headers=["TTS Service", "First-Token Latency (s)", "Status"], tablefmt="grid"))
        print("\nTest Notes: measures the time from establishing the connection to receiving the first audio data chunk (including handshake, authentication, and sending text), averaged over multiple runs")
        print("- Timing start: before the WebSocket/HTTP connection is established (includes network connection, handshake, and sending text)")
        print("- Timeout control: maximum wait time for a single request is 10 seconds")
        print("- Error handling: failed tests are not included in the average; only successful test latencies are counted")
    ...
    async def run(self, test_text=None, test_count=5):
        ...
        Args:
            ...
            test_count: the number of test iterations for each TTS service
        """
        test_text = test_text or self.test_texts[0]
        print(f"Starting streaming TTS first-token latency test...")
        print(f"Test text: {test_text}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Streaming TTS first-token latency test tool")
    parser.add_argument("--text", help="the text content to test")
    parser.add_argument("--count", type=int, default=5, help="the number of test iterations for each TTS service")
