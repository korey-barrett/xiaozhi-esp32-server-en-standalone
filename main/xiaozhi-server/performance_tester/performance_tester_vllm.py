...
description = "Vision Model Performance Test"
...
class AsyncVisionPerformanceTester:
    ...
    async def _test_vllm(self, vllm_name: str, config: Dict) -> Dict:
        ...
        try:
            ...
            if "api_key" in config and any(
                ...
                print(f"⏭️  VLLM {vllm_name} does not have an api_key configured, skipped")
            ...
            if not valid_results:
                print(f"⚠️  {vllm_name} no valid data, possibly a configuration error")
                return {"name": vllm_name, "type": "vllm", "errors": 1}
            ...
            if len(filtered_times) < len(test_tasks) * 0.5:
                print(f"⚠️  {vllm_name} not enough valid data, the network may be unstable")
                return {"name": vllm_name, "type": "vllm", "errors": 1}
    ...
    async def run(self):
        ...
        if not self.test_images:
            print(f"\n⚠️  {self.image_root} path contains no image files, unable to test")
            return
