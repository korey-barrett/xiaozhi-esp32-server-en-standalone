...
play_music_function_desc = {
    ...
    "function": {
        "name": "play_music",
        "description": "Call when the user asks to play music or a song.",
        "parameters": {
            "type": "object",
            "properties": {
                "song_name": {
                    "type": "string",
                    "description": "The name of the song. If the user did not specify a specific song name, use 'random'. When clearly specified, return the music name. Example: ```user: play Two Tigers\nparameter: Two Tigers``` ```user: play music \nparameter: random```",
                }
            },
            "required": ["song_name"],
        },
...
async def play_music(conn: "ConnectionHandler", song_name: str):
    try:
        ...
        return ActionResponse(
            action=Action.RECORD, result="Command received", response="Now playing music for you"
        )
...
async def handle_music_command(conn: "ConnectionHandler", text):
    ...
    conn.logger.bind(tag=TAG).debug(f"Checking whether it is a music command: {clean_text}")
    ...
    if os.path.exists(MUSIC_CACHE["music_dir"]):
        ...
        if potential_song:
            ...
            if best_match:
                conn.logger.bind(tag=TAG).info(f"Found the best matching song: {best_match}")
                await play_local_music(conn, specific_file=best_match)
                return True
    # Check whether it is a general play-music command
    await play_local_music(conn)
    return True


def _get_random_play_prompt(song_name):
    """Generate a random play intro message"""
